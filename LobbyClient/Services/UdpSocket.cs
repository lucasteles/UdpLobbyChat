...
{
    public const int MaxUdpPacketSize = 65_527;
    ...
    public UdpSocket(IPEndPoint bindEndpoint)
    {
        anyEndPoint = new(bindEndpoint.AddressFamily switch
        {
            ...
            _ => throw new InvalidOperationException(
                $"Invalid binding endpoint address family {bindEndpoint.AddressFamily}"),
        }, IPEndPoint.MinPort);
    ...
    public static Memory<byte> GetMemoryBuffer()
    {
        var pinnedBuffer = GC.AllocateArray<byte>(MaxUdpPacketSize, pinned: true);
        return MemoryMarshal.CreateFromPinnedArray(pinnedBuffer, 0, pinnedBuffer.Length);
    }
}
